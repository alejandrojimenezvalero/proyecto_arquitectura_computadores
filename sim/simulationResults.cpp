...
void writeParticleData(const std::string& filename, SimulationData data) {
    ...
    if (!outputFile) {throwException( "Cannot open " + filename + " for writing", -4);}
    //En el archivo se deben escribir primero los datos ppm y np
    auto num_particles = static_cast<float>(data.np);
    ...
    for (const auto& particle : grid_particles) {
      ...
      float velx = static_cast<float>(particle.vel[0]);float vely = static_cast<float>(particle.vel[0]);float velz = static_cast<float>(particle.vel[0]);

      outputFile.write(reinterpret_cast<const char*>(&posx), sizeof(posx));
      outputFile.write(reinterpret_cast<const char*>(&posy), sizeof(posy));
      outputFile.write(reinterpret_cast<const char*>(&posz), sizeof(posz));
      outputFile.write(reinterpret_cast<const char*>(&hvx), sizeof(hvx));
      outputFile.write(reinterpret_cast<const char*>(&hvy), sizeof(hvy));
      outputFile.write(reinterpret_cast<const char*>(&hvz), sizeof(hvz));
      outputFile.write(reinterpret_cast<const char*>(&velx), sizeof(velx));
      outputFile.write(reinterpret_cast<const char*>(&vely), sizeof(vely));
      outputFile.write(reinterpret_cast<const char*>(&velz), sizeof(velz));
    }
