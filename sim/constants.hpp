...
namespace simulationConstants {
  ...
  const double PI = M_PI;                                       //NOLINT
  const double MINIMUM_VARIATION = pow(10,-10);
  const double MINIMUM_DISTANCE = pow(10, -12);
  const std::vector<double> GRAVITY = {0.0, -9.8, 0.0};         //NOLINT
  const std::vector<double> UPPER_LIMIT{0.065, 0.1, 0.065};     //NOLINT
  const std::vector<double> LOWER_LIMIT{-0.065,-0.08, -0.065};  //NOLINT
}
