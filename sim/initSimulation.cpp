...
void initializeData(SimulationData &data, const double smoothing_length, const double particle_mass) {
    ...
    // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
    data.escalar_pos = (15 / (simulationConstants::PI * data.smoothing_length_6)) *
                       ((3 * particle_mass * simulationConstants::STIFFNESS_PRESSURE) / 2);
    // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
...
void calculateParameters(double ppm, int np, SimulationData &data) {
    ...
    std::cout << "Grid size: " << data.grid.grid_dimensions[0] << " x " << data.grid.grid_dimensions[1] << " x "
              << data.grid.grid_dimensions[2] << '\n';
...
void readParticleFields(std::ifstream &input_file, Particle &particle) {
    float floatField = 0;
    for (const auto &field: {particle.pos.data(), &particle.pos[1], &particle.pos[2], particle.hv.data(),
                             &particle.hv[1], &particle.hv[2], particle.vel.data(), &particle.vel[1],
                             &particle.vel[2]}) {
...
int setParticleData(std::ifstream &input_file, SimulationData &data) {
    ...
    if (data.np != real_particles) {
        throwException("Error: Number of particles mismatch. Header:  " + std::to_string(data.np) + ", Found: " +
                       std::to_string(real_particles),
...
int initiateSimulation(const std::vector<std::string> &stringInitVector) {
    ...
    float ppmFloat = 0.0;
    //NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    input_file.read(reinterpret_cast<char *>(&ppmFloat),
                    sizeof(float));
    const auto ppm = static_cast<double>(ppmFloat);
    int num_particles = 0;
    //NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    input_file.read(reinterpret_cast<char *>(&num_particles),
                    sizeof(int));
    if (num_particles < 0) {
        throwException("Error: Invalid number of particles: " + std::to_string(num_particles),
                       -5);//NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
    }
    SimulationData data;
